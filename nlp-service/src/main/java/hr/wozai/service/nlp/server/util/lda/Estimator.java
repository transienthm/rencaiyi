...
public class Estimator {
	...
	private Logger logger = LoggerFactory.getLogger(Estimator.class);

	// output model
	protected Model trnModel;
	LDAOption option;
	
	public boolean init(LDAOptionModel option, List<String> allTrainData){
		this.option = new LDAOption();
		BeanHelper.copyPropertiesHandlingJSONIgnoreNull(option, this.option);
		trnModel = new Model();

		if (this.option.est){
			if (!trnModel.initNewModel(this.option, allTrainData))
				return false;
	...
	public void estimate(){
		this.logger.info("Sampling " + trnModel.niters + " iteration!");
		
		int lastIter = trnModel.liter;
		for (trnModel.liter = lastIter + 1; trnModel.liter < trnModel.niters + lastIter; trnModel.liter++){
			this.logger.info("Iteration " + trnModel.liter + " ...");
			...
			if (option.savestep > 0){
				if (trnModel.liter % option.savestep == 0){
					this.logger.info("Saving the model at iteration " + trnModel.liter + " ...");
					computeTheta();
					computePhi();
		...
		this.logger.info("Gibbs sampling completed!");
		this.logger.info("Saving the final model!");
		computeTheta();
		computePhi();
		trnModel.liter--;
