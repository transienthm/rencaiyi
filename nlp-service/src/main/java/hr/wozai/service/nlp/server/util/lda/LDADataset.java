...
public class LDADataset {
	...
	private static Logger LOGGER = LoggerFactory.getLogger(LDADataset.class);
	 ...
	 */
	public static LDADataset readDataSet(List<String> allTrainData){
		try {
			//BufferedReader reader = new BufferedReader(new InputStreamReader(
			//		new FileInputStream(filename), "UTF-8"));
			
			LDADataset data = readDataSetAnother(allTrainData);
			
			//reader.close();
			return data;
		}
		catch (Exception e){
			LOGGER.info("Read Dataset Error: " + e.getMessage());
			e.printStackTrace();
			return null;
		}
	}
	 ...
	 * @param forecastData file from which we read dataset
	 * @param dict the dictionary
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSet(List<String> forecastData, Dictionary dict){
		try {
			//BufferedReader reader = new BufferedReader(new InputStreamReader(
			//		new FileInputStream(filename), "UTF-8"));
			LDADataset data = readDataSetAnotherDict(forecastData, dict);
	 ...
	 *  @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSetAnother(List<String> allTrainData){
		try {
			//read number of document
			String line;
			//line = reader.readLine();
			int M = allTrainData.size();
			
			LDADataset data = new LDADataset(M);
			for (int i = 0; i < M; ++i){
				line = allTrainData.get(i);
				data.setDoc(line, i);
			}
	 ...
	 * @param forecastData stream from which we read dataset
	 * @param dict the dictionary
	 * @return dataset if success and null otherwise
	 */
	public static LDADataset readDataSetAnotherDict(List<String> forecastData, Dictionary dict){
		try {
			//read number of document
			String line;
			//line = reader.readLine();
			int M = forecastData.size();
			LOGGER.info("NewM:" + M);
			
			LDADataset data = new LDADataset(M, dict);
			for (int i = 0; i < M; ++i){
				line = forecastData.get(i);
				data.setDoc(line, i);
			}
	...
	public static LDADataset readDataSet(String [] strs, Dictionary dict){
		//this.logger.info("readDataset...");
		LDADataset data = new LDADataset(strs.length, dict);
		
		for (int i = 0 ; i < strs.length; ++i){
			//this.logger.info("set doc " + i);
			data.setDoc(strs[i], i);
		}
		return data;
	}
